...
def concat_multiple_edits(data, sample):
    ...
    """
    # define output files
    concat1 = os.path.join(data.tmpdir, f"{sample.name}_concat_edits_R1.fastq.gz")
    concat2 = os.path.join(data.tmpdir, f"{sample.name}_concat_edits_R2.fastq.gz")
    ...
    if len(read1s) > 1:
        ...
        with open(concat1, 'w') as cout:
            proc = subprocess.Popen(
                cmd, stderr=subprocess.STDOUT, stdout=cout, close_fds=True)
            res = proc.communicate()[0]
            if proc.returncode:
                raise IPyradError("error in: {} {}".format(cmd, res))
...
def merge_pairs_with_vsearch(data, sample):
    ...
    in1 = [
        ...
        os.path.join(data.tmpdir, f"{sample.name}_concat_edits_R1.fq.gz"),
        sample.files.edits[0][0],        
    ]
    in2 = [
        os.path.join(data.tmpdir, f"{sample.name}_unmapped_R2.fastq"),
        os.path.join(data.tmpdir, f"{sample.name}_concat_edits_R2.fq.gz"),
...
def merge_end_to_end(data, sample, revcomp, append, identical=False):
    ...
    # input file options
    altmapped1 = os.path.join(data.tmpdir, "{sample.name}_unmapped_R1.fastq")
    altmapped2 = os.path.join(data.tmpdir, "{sample.name}_unmapped_R2.fastq")
    nonmerged1 = os.path.join(data.tmpdir, "{sample.name}_nonmerged_R1.fastq")
    nonmerged2 = os.path.join(data.tmpdir, "{sample.name}_nonmerged_R2.fastq")
    concat1 = os.path.join(data.tmpdir, "{sample.name}_concat_edits.fastq.gz")
    concat2 = os.path.join(data.tmpdir, "{sample.name}_concat_edits.fastq.gz")
