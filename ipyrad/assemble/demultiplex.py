...
def make_stats(data, perfile, fsamplehits, fbarhits, fmisses, fdbars):
    ...
    for sname in snames:
        ...
        if 'pair' in data.paramsdict["datatype"]:
            sample.files.fastqs = [(
                os.path.join(data.dirs.fastqs, sname + "_R1_.fastq.gz"),
                os.path.join(data.dirs.fastqs, sname + "_R2_.fastq.gz"), 
                )]
        else:
            sample.files.fastqs = [
                (os.path.join(data.dirs.fastqs, sname + "_R1_.fastq.gz"), "")]
...
def barmatch(data, tups, cutters, longbar, matchdict, fnum):
    ...
    ip.logger.debug("Doing chunk %s", tups[0])
    ...
    while 1:
        ...
        if '3rad' not in data.paramsdict["datatype"]:
            ...
            barcode1 = find3radbcode(
                cutters=cutters, 
                longbar=longbar, 
                read1=read1)
            barcode2 = find3radbcode(
                cutters=cutters, 
...
def concat_chunks(data, ipyclient):
    ...
    #printstr = ' writing/compressing   | {} | s1 |'
    # .format(elapsed), spacer=data._spacer) 
    ...
    for sname in set(snames):
        ...
        writers.append(
            lbview.apply(collate_files, *[data, sname, tmp1s, tmp2s]))

    total = len(writers)
    while 1:
        ready = [i.ready() for i in writers]
        data._progressbar(total, sum(ready), start, printstr)
        time.sleep(0.1)
        if all(ready):
            break
    print("")
...
#     proc = sps.Popen(['which', 'pigz'], 
#            stderr=sps.PIPE, stdout=sps.PIPE).communicate()
#     if proc[0].strip():
#         compress = ["pigz", "-p", "4"]
#     else:
#         compress = ["gzip"]
...
#     ip.logger.info('ncpus %s', data.cpus)
#     ip.logger.info('optim %s', optim)
...
#     #lbview = ipyclient.load_balanced_view()
#     tiny = ipyclient.load_balanced_view(targets=ipyclient.ids[:4])
...
#     ### progress
#     ip.logger.info("chunks size = %s lines, on %s cpus", optim, data.cpus)
...
#     ip.logger.info("chunkfiles %s", chunkfiles)
#     for fidx, tuplist in chunkfiles.items():
#         for tups in tuplist:
#             ip.logger.info("tups %s", tups)
#             args = [data, tups, cutters, longbar, matchdict, fidx]
#             filesort[fidx].append(tiny.apply(barmatch, *args))
...
#             rasyncs = [i for (i, j) in zip(allsyncs, ready) if j]
#             for rasync in rasyncs:
#                 result = rasync.result()
#                 if result:
#                     putstats(result, raws[fidx][0], statdicts)
#                     ## purge from memory
#                     ipyclient.purge_local_results(rasync)
#                     done += 1
#                     filesort[fidx].remove(rasync)
...
#         tmp1s = sorted(r1dict[sname])
#         tmp2s = sorted(r2dict[sname])
#         writers.append(tiny.apply(collate_files, *[data, sname, tmp1s, tmp2s]))
