    ...
    try: 
        ...
        if preview:
            LOGGER.warn("Running preview mode. Selecting subset of"\
                        " data for demultiplexing.")
            sample_fastq = [raws[0][0]]
            if "pair" in data.paramsdict["datatype"]:
                sample_fastq += [raws[0][1]]
            sample_fastq = preview_truncate_fq(data, sample_fastq)

            ## emulating the format output by prechecks, and expected by
            ## downstream, a list of tooples, with 0 for second read if SE
            raws = [(sample_fastq[0], 0)]

            if "pair" in  data.paramsdict["datatype"]:
                raws = [(raws[0][0],sample_fastq[1])]
